...
//  ROS_INFO("[YoloObjectDetector] numClasses is %d",numClasses_);
  rosBoxes_ = std::vector<std::vector<RosBox_> >(numClasses_);
  rosBoxCounter_ = std::vector<int>(numClasses_);
//  ROS_INFO("[YoloObjectDetector] rosBoxes_ size is %d",rosBoxes_[0].size());
//  ROS_INFO("[YoloObjectDetector] rosBoxCounter_ size is %d",rosBoxCounter_.size());
...
void YoloObjectDetector::init()
{
  ...
  nodeHandle_.param("subscribers/camera_reading/topic", cameraTopicName,   
                    std::string("/camera/image_raw"));  //subscribers/camera_reading/topic
...
{
  ROS_INFO("ENTER IN fetchInThread()");
...
{
  show_image_cv(buff_[(buffIndex_ + 1) % 3], "Demo", ipl_);//zhanghm：调用darknet/src文件夹下的show_image_cv函数
...
void YoloObjectDetector::yolo()
{
  ...
  IplImage* ROS_img = getIplImage();//zhanghm,先尝试从图像序列中获取一张图片，用来初始化一些变量
  ...
  while (!demoDone_) {//zhanghm：只要没有停止节点运行,并且没有在视频框中按下ESC键,则一直循环检测
    buffIndex_ = (buffIndex_ + 1) % 3;
    fetch_thread = std::thread(&YoloObjectDetector::fetchInThread, this);//获取图像
    detect_thread = std::thread(&YoloObjectDetector::detectInThread, this);//检测目标并绘制目标框
    if (!demoPrefix_) {
