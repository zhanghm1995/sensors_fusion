...
// PCL specific part
  ...
  //zhanghm add 20180131: read pcl::PointXYZI data
  static Cloud::Ptr FromPclXYZI(const pcl::PointCloud<pcl::PointXYZI>& pcl_cloud) {
    Cloud cloud;
    for (const auto& pcl_point : pcl_cloud) {
      //zhanghm 20180130: change for Toyota lidar
      //RichPoint point(pcl_point.x, pcl_point.y, pcl_point.z);
      if(pcl_point.intensity<0.01f) //filter the invalid points
       {
           continue;
       }
      RichPoint point(pcl_point.y, -pcl_point.x, pcl_point.z);
