...
Cloud::Cloud(const Cloud& cloud) //复制构造函数
...
Cloud::Ptr Cloud::FromImage(const cv::Mat& image,
  ...
  CloudProjection::Ptr proj = CloudProjection::Ptr(new RingProjection(params));//首先根据构造的深度图像参数确定投影类中成员
  proj->CheckImageAndStorage(image);//check image data format and params 然后验证输入的深度图像是否和事先确定的投影参数一致
  proj->CloneDepthImage(image);//image -> proj member variable of _depth_image
  Cloud cloud;
...
Cloud::Ptr Cloud::FromPcl(const pcl::PointCloud<pcl::PointXYZL>& pcl_cloud) {
  ...
  for (const auto& pcl_point : pcl_cloud) {
    //zhanghm 20180130: change for Toyota lidar
    //RichPoint point(pcl_point.x, pcl_point.y, pcl_point.z);
    RichPoint point(pcl_point.y, -pcl_point.x, pcl_point.z);
