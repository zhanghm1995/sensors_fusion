...
cv::Mat MatFromDepthPng(const string& path) {
  cv::Mat depth_image = cv::imread(path, CV_LOAD_IMAGE_ANYDEPTH);//actually the load image type is CV_8UC3

  depth_image.convertTo(depth_image, CV_32F); //convert the image data type to CV_32F
  depth_image /= 500.;
